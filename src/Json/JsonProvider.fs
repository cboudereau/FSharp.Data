...
type public JsonProvider(cfg:TypeProviderConfig) as this =
    ...
    let buildTypes (typeName:string) (args:obj[]) =
        ...
        let globalInference = args.[8] :?> bool
        ...
        let getSpec _ value = 
            ...
            let inferedType = using (IO.logTime "Inference" sample) <| fun _ ->
                samples
                |> JsonInference.inferTypes inferTypesFromValues cultureInfo rootName globalInference
            
            using (IO.logTime "TypeGeneration" sample) <| fun _ ->
            
                let ctx = JsonGenerationContext.Create(cultureStr, tpType, unifyGlobally=globalInference)
    ...
    let helpText = 
        """<summary>Typed representation of a JSON document.</summary>
           ...
           <param name='Global'>If true, the inference unifies all Json Values with the same name.</param>"""
